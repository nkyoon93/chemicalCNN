...
while v <= args.validation:
    ...
    test_iter = chainer.iterators.SerialIterator(dataset[v - 1][1], batch_size=args.batchsize, repeat=False,
                                                 shuffle=False)
    ...
    trainer.extend(extensions.snapshot_object(model, 'model_' + str(v) + '_snapshot_{.updater.epoch}'),
                   trigger=(frequency, 'epoch'))
    trainer.extend(extensions.snapshot_object(optimizer, 'optimizer_' + str(v) + '_snapshot_{.updater.epoch}'),

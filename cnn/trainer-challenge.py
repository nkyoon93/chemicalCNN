...
def main():
    ...
    parser.add_argument('--epoch', '-e', type=int, default=500,
                        help='Number of sweeps over the dataset to train. Default = 500')
    ...
    parser.add_argument('--atomsize', '-a', type=int, default=400,
                        help='Max length of smiles, SMILES which length is larger than this value will be skipped. Default = 400')
    ...
    for mol in mols:
        if len(Chem.MolToSmiles(mol, kekuleSmiles=True, isomericSmiles=True)) > args.atomsize:
            print("SMIELS is too long. This mol will be ignored.")
        else:
            F_list.append(mol_to_feature(mol, -1, args.atomsize))
            T_list.append(mol.GetProp('_Name'))
    Mf.random_list(F_list)
